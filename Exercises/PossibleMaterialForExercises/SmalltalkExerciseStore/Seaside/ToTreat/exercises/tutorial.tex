...
Follow the instructions given on the slides to install Seaside. Make sure your Seaside server is up and running by accessing the example application at \code{http://\-localhost:8080/\-seaside/\-counter} in Squeak or at \code{http://\-localhost:8008/\-seaside/\-go/\-counter} in VisualWorks.

In Squeak load the monticello package \code{tutorial.mcz} and in VisualWorks the parcel \code{tutorial.pcl}. Both packages contain examples shown during the presentation and some class skeletons that will assist you to do these exercises.
...
\exercise{Change the behaviour of the increase and decrease buttons: edit the methods \method{increase} and \method{decrease} from within the web browser to increase by \code{2} and decrese by \code{3}.}

\exercise{Inspect the living component from within the web browser. There are two instance variables visible, whereas \code{count} is representing the state of the component. The other instance variable is defined in a super-class of \class{WACounter} and will be discussed later on.}

\exercise{Change the background color of the web application by using the style editor from within your web browser. Try using something like \code{body \{ background-color: yellow; \}}.}
...
\item Create an initialization method on the class side of the newly created class, registering the component as a new web application with the path segment \code{cng}.
...
\exercise{Register \class{STTicTacToeController} as a new web-application, but this time don't use a class initialization method but the configuration interface. Make sure that you have a method \method{canBeRoot} on the class-side so that Seaside recognizes this class as a possible root of a web application. Browse to \code{http://\-localhost:8080/\-seaside/\-config} when using Squeak or \code{http://\-localhost:8008/\-seaside/\-go/\-config} when using VisualWorks, enter your password, add a new entry point with the name \code{ttt} and select \class{STTic\-Tac\-Toe\-Controller} as the root component.}
...
\begin{figure}[htbp]
\begin{center}
	\includegraphics[scale=0.6]{../grafics/theater-model.pdf}
...
\begin{figure}[htbp]
\begin{center}
	\includegraphics[scale=0.6]{../grafics/theater-flow.pdf}
...
\begin{figure}[htbp]
\begin{center}
	\includegraphics[scale=0.5]{../grafics/theater-showchooser.pdf}
...
\begin{figure}[htbp]
\begin{center}
	\includegraphics[scale=0.5]{../grafics/theater-ticketchooser.pdf}
...
\begin{figure}[htbp]
\begin{center}
	\includegraphics[scale=0.5]{../grafics/theater-ticketprinter.pdf}
...
\begin{figure}[htbp]
\begin{center}
	\includegraphics[scale=0.5]{../grafics/theater-mainframe.pdf}
...
\begin{figure}[htbp]
\begin{center}
	\includegraphics[scale=0.5]{../grafics/theater-showreport.pdf}
...
\begin{figure}[htbp]
\begin{center}
	\includegraphics[scale=0.5]{../grafics/theater-editplay.pdf}
...
\exercise{Implement \method{updateUrl:} in the three top-level sub-components of your web application and add an appropriate path-element to the URL. Depending on the context of your application, the URL should now look like: \code{../theater/buy}, \code{../theater/change} and \code{../theater/report}.}
...
%
%This is a human-readable summary of the Legal Code (the full license) available at \code{http://creativecommons.org/\-licenses/\-by-sa/\-2.0/\-legalcode}.
